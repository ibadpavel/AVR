...
#define zetaDef sqrt(3.0f / 4.0f) * gyroMeasDrift           // compute zeta
...
void MadgwickOptim::update(float w_x, float w_y, float w_z, float a_x, float a_y, float a_z, float m_x, float m_y, float m_z) {
	// Use IMU algorithm if magnetometer measurement invalid (avoids NaN in magnetometer normalisation)
    if((m_x == 0.0f) && (m_y == 0.0f) && (m_z == 0.0f)) {
        updateIMU(w_x, w_y, w_z, a_x, a_y, a_z);
		return;
	}
    ...
    w_bx += w_err_x * invSampleFreq * zetaDef;
    w_by += w_err_y * invSampleFreq * zetaDef;
    w_bz += w_err_z * invSampleFreq * zetaDef;
    w_x -= w_bx;
    w_y -= w_by;
    w_z -= w_bz;
    ...
    SEq_1 += (SEqDot_omega_1 - (betaDef * SEqHatDot_1)) * invSampleFreq;
    SEq_2 += (SEqDot_omega_2 - (betaDef * SEqHatDot_2)) * invSampleFreq;
    SEq_3 += (SEqDot_omega_3 - (betaDef * SEqHatDot_3)) * invSampleFreq;
    SEq_4 += (SEqDot_omega_4 - (betaDef * SEqHatDot_4)) * invSampleFreq;
...
void MadgwickOptim::updateIMU(float w_x, float w_y, float w_z, float a_x, float a_y, float a_z) {
	// Convert gyroscope degrees/sec to radians/sec
    w_x *= 0.0174533f;
    w_y *= 0.0174533f;
    w_z *= 0.0174533f;
...
void MadgwickOptim::computeAngles()
{
    roll = atan2f(SEq_1 * SEq_2 + SEq_3 * SEq_4, 0.5f - SEq_2 * SEq_2 - SEq_3 * SEq_3);
    pitch = asinf(-2.0f * (SEq_2 * SEq_4 - SEq_1 * SEq_3));
    yaw = atan2f(SEq_2 * SEq_3 + SEq_1 * SEq_4, 0.5f - SEq_3 * SEq_3 - SEq_4 * SEq_4);
	anglesComputed = 1;
}
